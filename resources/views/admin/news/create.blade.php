...
@section('content')
    ...
    <div class="bg-white p-6 shadow-sm max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
            <h2 class="font-primary font-bold text-lg text-[#1a2c42]">Novo Artigo</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="form-group">
                    ...
                    <select id="category" name="category" 
                        ...
                        <option value="Corporativo" {{ old('category') === 'Corporativo' ? 'selected' : '' }}>Corporativo</option>
                        ...
                        <option value="Enviro" {{ old('category') === 'Enviro' ? 'selected' : '' }}>Enviro</option>
                    </select>
                    @error('category')
                        <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
                    @enderror
                ...
                <!-- Cover Image -->
                <div class="form-group">
                    <label for="cover_image" class="block font-primary text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Imagem de Destaque</label>
                ...
                <div class="form-group md:col-span-2">
                    ...
                    <textarea id="summary" name="summary" rows="3" 
                              class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 focus:outline-none focus:border-[#e51718] focus:ring-1 focus:ring-[#e51718] text-sm" placeholder="Resumo curto para exibições de lista..." required>{{ old('summary') }}</textarea>
                    @error('summary')
                        <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
                    @enderror
                </div>
